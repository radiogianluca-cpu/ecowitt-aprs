...
def normalize_temp(temp):
    ...
    # se sembra Fahrenheit (tipico Ecowitt se > 45°C equivalente)
    if temp > 60:
        return (temp - 32) * 5 / 9
...
# ======================
# PRESSURE FIX (Ecowitt variability)
# ======================
def normalize_pressure(p):
    p = to_float(p)

    # valori troppo bassi = unità diversa o errore
    if p < 900:
        # tentativi realistici Ecowitt
        p = p * 3.38639  # fallback inHg → hPa approx
...
def build_packet(data):
    ...
    # RAW VALUES
    temp = normalize_temp(outdoor.get("temperature"))
