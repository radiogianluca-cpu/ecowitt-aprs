...
def build_packet(data):
    ...
    temp_c = safe_value(outdoor.get("temperature"))
    humidity = safe_value(outdoor.get("humidity"))
    baro = safe_value(pressure.get("relative"))

    wind_speed = safe_value(wind.get("wind_speed"))
    wind_dir = safe_value(wind.get("wind_direction"))
...
def send_aprs(packet):
    ...
    login = f"user {CALLSIGN} pass {PASSCODE} vers ecowitt-api 1.0\n"
    s.send(login.encode())
