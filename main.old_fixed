...
def normalize_temp(outdoor):
    ...
    - °C
    - °F
    - campi Ecowitt diversi (temperature / tempf)
    """
    temp = outdoor.get("temperature")

    # fallback Ecowitt alternative field
    if temp is None:
        temp = outdoor.get("tempf")
...
def normalize_pressure(p):
    ...
    if p < 900:
        p = p * 3.38639  # fallback Ecowitt inHg → hPa
...
def run():
    try:
        ...
        # DEBUG (puoi lasciarlo o togliere dopo)
        print(json.dumps(data.get("data", {}).get("outdoor", {}), indent=2))
